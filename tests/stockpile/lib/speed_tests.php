...
for( $i = 0; $i < $iterations; $i++) $stockpile->add( $i + 1, uniqueNumber(1,10000));
...
$start = microtime(TRUE);
for( $i = 0; $i < $iterations; $i++) $stockpile->get( $i + 1);
$elapsed = microtime(TRUE) - $start;
...
$stockpile = stockpile( $app, $user_id );
$start = microtime(TRUE);
for( $i = 0; $i < $iterations; $i+=100) $stockpile->get( range($i + 1, $i+101) );
$elapsed = microtime(TRUE) - $start;
