...
class PDO extends Callback implements \Gaia\DB\Iface {
    ...
    public function __construct( $dsn ){
        ...
        $callbacks['format_query_args'] = $format_args = function($query, array $args ) use ( $db ){
        ...
        $callbacks['isa'] = function( $name ) use( $wrapper, $db ) {
         ...
         $callbacks['execute'] = function( $query ) use ( $db, $format_args ){
            $args = func_get_args();
            array_shift($args);
            $rs = $db->query( $sql = $format_args( $query, $args ) );
            return $rs;
        };
